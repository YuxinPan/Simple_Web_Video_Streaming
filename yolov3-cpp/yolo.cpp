...
int main()
{
    ...
    std::string modelConfiguration = pathNN + "yolov3-tiny.cfg";
    std::string modelWeights = pathNN + "yolov3-tiny.weights";
    ...
    while (1) {
        ...
        try
        {
            ...
            // Open the image file
            outputFile = pathImage + std::to_string(newestTimestamp) + imgExt;
            std::cout << std::endl << std::endl << "Input: " << outputFile << std::endl;
            if (outputFile.compare(lastFile) == 0) { // if same as last file then
                std::this_thread::sleep_for(std::chrono::milliseconds(sleep_interval)); // sleep in the loop
                continue;
            }
            lastFile = outputFile;
            ...
            blobFromImage(frame, blob, 1 / 255.0, cv::Size(inpWidth, inpHeight), Scalar(0, 0, 0), true, false, CV_32F);
